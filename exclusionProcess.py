...
def sample(initialState,initialParticleStates,numberofParticles,T):
    states = initialState.copy()
    particleStates = initialParticleStates.copy()
    numberOfParticles = len(initialParticleStates)
    sum = 0
    t=0
...
def simulate(n,T):
    ...
    for x in range(0,N):
        rand = bool(random.getrandbits(1))
        initialState.append(rand)
        if rand == True:
            initialParticleStates.append(x)
            
    for x in range(0,n):
        samples.append(sample(initialState,initialParticleStates,len(initialParticleStates),T))
